...
function buddyforms_gdpr_shortcode_data_request( $atts ) {
	...
	if ( function_exists( 'wp_create_user_request' ) ) {
		...
		<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="buddyforms-gdpr-form">
			...
			<input type="hidden" name="buddyforms_gdpr_data_human_key" id="buddyforms-gdpr-data-human-key" value="<?php echo esc_attr( $number_one ) . '000' . esc_attr( $number_two ); ?>" />
			<input type="hidden" name="buddyforms_gdpr_data_nonce" id="buddyforms-gdpr-data-nonce" value="<?php echo esc_attr( wp_create_nonce( 'buddyforms_gdpr_nonce' ) ); ?>" />
			...
			<p class="buddyforms-gdpr-field buddyforms-gdpr-field-human">
				<label for="buddyforms-gdpr-data-human">
					...
					<?php echo esc_attr( $number_one . ' + ' . $number_two . ' = ?' ); ?>
				</label>
...
function buddyforms_gdpr_data_request() {
	...
	$buddyforms_gdpr_email     = sanitize_email( wp_unslash( $_POST['buddyforms_gdpr_data_email'] ) );
