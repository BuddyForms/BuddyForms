...
function buddyforms_gdpr_shortcode_data_request( $atts ) {
	...
	if ( function_exists( 'wp_create_user_request' ) ) {
        ...
        <form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="buddyforms-gdpr-form">
            ...
            <input type="hidden" name="buddyforms_gdpr_data_human_key" id="buddyforms-gdpr-data-human-key"
                   value="<?php echo $number_one . '000' . $number_two; ?>"/>
            <input type="hidden" name="buddyforms_gdpr_data_nonce" id="buddyforms-gdpr-data-nonce"
                   value="<?php echo wp_create_nonce( 'buddyforms_gdpr_nonce' ); ?>"/>

            <div class="buddyforms-gdpr-field buddyforms-gdpr-field-action" role="radiogroup"
                 aria-labelledby="buddyforms-gdpr-radio-label">
                <p id="buddyforms-gdpr-radio-label">
					<?php esc_html_e( 'Select your request:', 'buddyforms' ); ?>
                </p>

                <input id="buddyforms-gdpr-data-type-export" class="buddyforms-gdpr-data-type-input" type="radio"
                       name="buddyforms_gdpr_data_type" value="export_personal_data">
                ...
                <br/>
                <input id="buddyforms-gdpr-data-type-remove" class="buddyforms-gdpr-data-type-input" type="radio"
                       name="buddyforms_gdpr_data_type" value="remove_personal_data">
            ...
            <p class="buddyforms-gdpr-field buddyforms-gdpr-field-submit">
                <input id="buddyforms-gdpr-submit-button" type="submit"
                       value="<?php esc_html_e( 'Send request', 'buddyforms' ); ?>"/>
            </p>
        </form>
		<?php
		return ob_get_clean();
