...
function buddyforms_wp_login_errors_redirect( $errors ) {
	...
	if ( $pagenow !== 'wp-login.php' || ( isset( $_SERVER['REQUEST_METHOD'] ) && $_SERVER['REQUEST_METHOD'] !== 'POST' ) ) {
		return $errors;
	}

	$login_page         = esc_url_raw( wp_unslash( $_POST['login_error_redirect'] ) );
...
function buddyforms_post_status_readable( $post_status ) {
	echo esc_html( buddyforms_get_post_status_readable( $post_status ) );
}
...
function buddyforms_post_status_css_class( $post_status, $form_slug ) {
	echo esc_attr( buddyforms_get_post_status_css_class( $post_status, $form_slug ) );
}
...
function buddyforms_upload_handle_dropped_media() {
	...
	if ( is_wp_error( $newupload ) ) {
		status_header( '500' );
		echo esc_html( $newupload->get_error_message() );
	} else {
		status_header( '200' );
		echo esc_html( $newupload );
	}
	die();
}
