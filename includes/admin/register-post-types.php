			...
			if ( $attached_page != 'Off' ) {
				...
				<div class="row-actions">
					<span class="view-form">
						<a target="_blank" href="<?php echo esc_url( wp_unslash( $attached_page_permalink ) ) . 'create/' . esc_url( wp_unslash( $post->post_name ) ); ?>"><?php esc_html_e( 'View Form', 'buddyforms' ); ?></a> |
					</span>
					<span class="view-entryies">
						<a target="_blank" href="<?php echo esc_url( wp_unslash( $attached_page_permalink ) ) . 'view/' . esc_url( wp_unslash( $post->post_name ) ); ?>"><?php esc_html_e( 'View Entries', 'buddyforms' ); ?></a>
					</span>
		...
		if ( get_post_type( $post ) == 'buddyforms' && ! isset( $_GET['wizard'] ) || isset( $_GET['wizard'] ) && $_GET['wizard'] != 'done' || ( isset( $_GET['post_type'] ) && $_GET['post_type'] == 'buddyforms' ) ) {
			...
			<script>
				jQuery(document).ready(function (jQuery) {
					...
					<?php
					$tmp = '<div id="buddyforms-adminhead-wizard" class="tk-icon-buddyforms"></div> BuddyForms';
		...
		<?php if ( isset( $post->post_name ) && $post->post_name != '' ) { ?>
			<div id="frontend-actions">
				<a class="button button-large bf_button_action" target="_blank"
				   href="<?php echo esc_url( wp_unslash( $base ) ); ?>/?page_id=<?php echo esc_html( wp_unslash( $preview_page_id ) ); ?>&preview=true&form_slug=<?php echo esc_html( wp_unslash( $post->post_name ) ); ?>"><span
		...
		<?php if ( isset( $buddyform['attached_page'] ) && isset( $buddyform['post_type'] ) && $buddyform['attached_page'] != 'none' ) { ?>
			<div class="bf-tile actions">
				<div id="frontend-actions">
					...
					?>
					<?php
					echo '<a class="button button-large bf_button_action" href="' . esc_url( wp_unslash( $attached_page_permalink ) ) . 'view/' . esc_html( wp_unslash( $post->post_name ) ) . '/" target="_new"><span class="dashicons dashicons-admin-page"></span> ' . esc_html__( 'Your Submissions', 'buddyforms' ) . '</a>
                <a class="button button-large bf_button_action" href="' . esc_url( wp_unslash( $attached_page_permalink ) ) . 'create/' . esc_html( wp_unslash( $post->post_name ) ) . '/" target="_new"><span class="dashicons dashicons-feedback"></span>    ' . esc_html__( 'The Form', 'buddyforms' ) . '</a>';
					?>
				</div>
			</div>
			<?php
		...
		if ( isset( $post->post_name ) && $post->post_name != '' ) {
			...
			<div class="bf-tile actions">
				<div id="admin-actions">
					...
					<?php echo '<a class="button button-large bf_button_action" href="edit.php?post_type=buddyforms&page=buddyforms_submissions&form_slug=' . esc_html( wp_unslash( $post->post_name ) ) . '"><span class="dashicons dashicons-email"></span> ' . esc_html__( 'Submissions', 'buddyforms' ) . '</a>'; ?>
				</div>
			</div>
		<?php } ?>
...
function buddyforms_export_form() {
	if ( isset( $_REQUEST['my_action'] ) && isset( $_REQUEST['post_id'] ) && 'export_form' == $_REQUEST['my_action'] ) {

		$buddyform_options = get_post_meta( sanitize_key( $_REQUEST['post_id'] ), '_buddyforms_options', true );
