...
function buddyforms_metabox_form_setup() {
	...
	$form_setup['Form Submission'][] = new Element_Url(
		'<b>' . __( 'Redirect URL', 'buddyforms' ),
		'buddyforms_options[after_submission_url]',
		array(
	...
	$form_setup['Form Submission'][] = new Element_Textarea(
		'<b>' . __( 'After Create Submission Message Text', 'buddyforms' ) . '</b>',
		'buddyforms_options[after_submit_message_text]',
		array(
			'rows'  => 3,
			'style' => 'width:100%',
	...
	$form_setup['Form Submission'][] = new Element_Textarea(
		'<b>' . __( 'After Update Submission Message Text', 'buddyforms' ) . '</b>',
		'buddyforms_options[after_update_submit_message_text]',
		array(
			'rows'  => 3,
			'style' => 'width:100%',
			'class' => 'display_message display_form',
	...
	$form_setup['Form Submission'][] = new Element_Textarea(
		'<b>' . __( 'Empty Submission List Message Text', 'buddyforms' ) . '</b>',
		'buddyforms_options[empty_submit_list_message_text]',
		array(
			'rows'      => 3,
			'style'     => 'width:100%',
	...
	$element = new Element_Checkbox(
		...
		array( 'bf_ajax' => __( 'Disable ajax form submission', 'buddyforms' ) ),
		array(
			'shortDesc' => '',
			'value'     => $bf_ajax,
		)
	...
	$element = new Element_Checkbox(
		...
		array( 'disable' => __( 'Disable Local Storage', 'buddyforms' ) ),
		array(
	...
	$element = new Element_Checkbox(
		...
		array( 'disabled' => __( 'Disable JavaScript Validation', 'buddyforms' ) ),
		array(
	...
	$element = new Element_Checkbox(
		...
		array( 'Revision' => __( 'Enable frontend revision control', 'buddyforms' ) ),
		array(
			'value' => $revision,
			'class' => 'bf_hide_if_post_type_none',
		)
	...
	$element = new Element_Textbox(
		'<b>' . __( 'Singular Name', 'buddyforms' ),
		'buddyforms_options[singular_name]',
		array(
			'value'     => $singular_name,
	...
	// Attached Page
	$form_setup['Edit Submissions'][] = new Element_HTML(
		sprintf( '<h4>%s</h4>', __( 'Enable your site members to view their submissions', 'buddyforms' ) ) .
	...
	$element = new Element_Checkbox(
		...
		array( 'Admin Bar' => __( 'Add to Admin Bar', 'buddyforms' ) ),
		array(
			'value' => $admin_bar,
	...
	if ( is_array( $form_setup ) ) {
		$form_setup = buddyforms_sort_array_by_Array(
			$form_setup,
			array(
	...
	<p>
		<select id="bf-form-type-select" style="margin-left: 0 !important;" name="buddyforms_options[form_type]">
			<optgroup label="Form Type">
				<option <?php selected( $form_type, 'contact' ); ?> value="contact"><?php esc_html_e( 'Contact Form', 'buddyforms' ); ?></option>
				<option <?php selected( $form_type, 'registration' ); ?> value="registration"><?php esc_html_e( 'Registration Form', 'buddyforms' ); ?></option>
				<option <?php selected( $form_type, 'post' ); ?> value="post"><?php esc_html_e( 'Post Form', 'buddyforms' ); ?></option>
			</optgroup>
		</select>
	</p>
	<div class="tabbable buddyform-tabs-left">
		<ul class="nav buddyform-nav-tabs buddyform-nav-pills">
			...
			foreach ( $form_setup as $tab => $fields ) {
				...
				?>
			<li class="<?php echo $i == 0 ? 'active' : ''; ?><?php echo esc_attr( $tab_slug ); ?>_nav">
				<a href="#<?php echo esc_attr( $tab_slug ); ?>" data-toggle="tab"><?php echo esc_attr( $tab ); ?></a>
				</li>
				<?php
				$i ++;
			}
