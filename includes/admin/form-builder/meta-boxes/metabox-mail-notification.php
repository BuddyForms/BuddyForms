...
function buddyforms_mail_notification_screen() {
	...
	$form_setup[] = new Element_HTML(
		'<h4>' . __( 'Mail Notifications', 'buddyforms' ) . '</h4>
...
function buddyforms_post_status_mail_notification_screen() {
	...
	$element = new Element_Select(
		'<h4>' . __( 'Post Status Change Mail Notifications', 'buddyforms' ) . '</h4><p>' . __( 'Forms can send different email notifications triggered by post status changes. For example, automatically notify post authors when their post is published! ', 'buddyforms' ) . '</p>',
		'buddyforms_notification_trigger',
...
function buddyforms_mail_notification_form( $trigger = false, $form_slug = '' ) {
	...
	$test_button  = sprintf(
		'<button class="button-primary btn btn-primary bf_test_trigger" id="%s" title="%s" data-form-slug="%s">%s</button><p class="description">%s</p>',
		$trigger,
		__( 'Test this Notification', 'buddyforms' ),
		$form_slug,
		__( 'Test', 'buddyforms' ),
	...
	$element      = new Element_Radio(
		'<b>' . __( 'From Name', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_submissions][' . $trigger . '][mail_from_name]',
		array(
	...
	$form_setup[] = new Element_Textbox(
		'<b>' . __( 'Custom Mail From Name', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_submissions][' . $trigger . '][mail_from_name_custom]',
		array(
	...
	// From email
	$element      = new Element_Radio(
		'<b>' . __( 'From email', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_submissions][' . $trigger . '][mail_from]',
		array(
	...
	$form_setup[] = new Element_Textbox(
		'<b>' . __( 'Custom Mail From Address', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_submissions][' . $trigger . '][mail_from_custom]',
		array(
	...
	$element           = new Element_Checkbox(
		'<b>' . __( 'Send mail to', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_submissions][' . $trigger . '][mail_to]',
		$send_mail_to_opts,
		array(
			'value' => $mail_to,
	...
	// to email
	$form_setup[] = new Element_Textbox(
		'<b>' . __( 'Custom sent mail to', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_submissions][' . $trigger . '][mail_to_address]',
		array(
			'class'     => 'bf-mail-field',
	...
	$form_setup[]  = new Element_Textbox(
		'<b>' . __( 'Subject', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_submissions][' . $trigger . '][mail_subject]',
		array(
			'class'     => 'bf-mail-field',
	...
	?>
	<li id="trigger<?php echo esc_attr( $trigger ); ?>" class="bf_trigger_list_item <?php echo esc_attr( $trigger ); ?>">
						...
						<tr>
							...
							<td class="field_label">
								<strong>
									<a class="bf_edit_field row-title accordion-toggle collapsed" data-toggle="collapse"
									   data-parent="#accordion_text" href="#accordion_<?php echo esc_attr( $trigger ); ?>"
									   title="Edit this Notification"
									   href="#"><?php echo isset( $buddyform['mail_submissions'][ $trigger ]['mail_subject'] ) ? wp_kses( $buddyform['mail_submissions'][ $trigger ]['mail_subject'], buddyforms_wp_kses_allowed_atts() ) : ''; ?></a>
								</strong>
							...
							<td class="field_delete">
								<span><a class="accordion-toggle collapsed" data-toggle="collapse"
										 data-parent="#accordion_text" href="#accordion_<?php echo esc_attr( $trigger ); ?>"
										 title="<?php esc_html_e( 'Edit this Notification', 'buddyforms' ); ?>" href="javascript:;"><?php esc_html_e( 'Edit', 'buddyforms' ); ?></a> | </span>
								<span><a class="bf_delete_trigger" id="<?php echo esc_attr( $trigger ); ?>" title="<?php esc_html_e( 'Delete this Notification', 'buddyforms' ); ?>"
										 href="javascript:;"><?php esc_html_e( 'Delete', 'buddyforms' ); ?></a></span>
							</td>
						</tr>
						</tbody>
					</table>
				</div>
				<div id="accordion_<?php echo esc_attr( $trigger ); ?>"
...
function buddyforms_new_post_status_mail_notification_form( $trigger, $form_slug = '' ) {
	...
	$form_setup[] = new Element_Textbox(
		...
		'buddyforms_options[mail_notification][' . $trigger . '][mail_from_name]',
		array(
	...
	$form_setup[] = new Element_Textbox(
		'<b>' . __( 'Email', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_notification][' . $trigger . '][mail_from]',
		array(
	...
	$form_setup[] = new Element_Checkbox(
		'<b>' . __( 'Send mail to', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_notification][' . $trigger . '][mail_to]',
		array(
	...
	$form_setup[] = new Element_Textbox(
		'<b>' . __( 'Send Mail To', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_notification][' . $trigger . '][mail_to_address]',
		array(
			'class'     => 'bf-mail-field',
	...
	$form_setup[] = new Element_Textbox(
		'<b>' . __( 'Subject', 'buddyforms' ) . '</b>',
		'buddyforms_options[mail_notification][' . $trigger . '][mail_subject]',
		array(
			'class'     => 'bf-mail-field',
	...
	<li id="trigger<?php echo esc_attr( $trigger ); ?>" class="bf_trigger_list_item <?php echo esc_attr( $trigger ); ?>">
		<div class="accordion_fields">
			<div class="accordion-group">
						...
						<tr>
							...
							<td class="field_label">
								<strong>
									<a class="bf_edit_field row-title accordion-toggle collapsed" data-toggle="collapse"
									   data-parent="#accordion_text" href="#accordion_<?php echo esc_attr( $trigger ); ?>"
									   title="Edit this Notification" href="#"><?php echo wp_kses( $trigger, buddyforms_wp_kses_allowed_atts() ); ?></a>
								</strong>
				...
				<div id="accordion_<?php echo esc_attr( $trigger ); ?>" class="accordion-body collapse">
					<div class="accordion-inner">
						<?php buddyforms_display_field_group_table( $form_setup, $trigger ); ?>
					</div>
				</div>
...
function buddyforms_new_post_status_mail_notification() {
	...
	try {
		...
		$trigger   = buddyforms_sanitize( '', wp_unslash( $_POST['trigger'] ) );
		$form_slug = buddyforms_sanitize_slug( wp_unslash( $_POST['form_slug'] ) );
...
function buddyforms_form_setup_nav_li_notification() {
	...
	<li class="notifications_nav"><a
		href="#notification"
		data-toggle="tab"><?php esc_html_e( 'Notifications', 'buddyforms' ); ?></a>
	</li>
	<?php
}
