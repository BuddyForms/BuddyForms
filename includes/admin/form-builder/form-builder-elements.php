...
function buddyforms_display_form_element( $args ) {
	...
	if ( isset( $_POST['fieldtype'] ) ) {
		$field_type = sanitize_text_field( wp_unslash( $_POST['fieldtype'] ) );
	}

	if ( isset( $_POST['unique'] ) ) {
		$field_unique = sanitize_text_field( wp_unslash( $_POST['unique'] ) );
	}
	...
	if ( buddyforms_core_fs()->is__premium_only() ) {
		if ( buddyforms_core_fs()->is_plan( 'professional' ) || buddyforms_core_fs()->is_trial() ) {
			...
			$form_fields['advanced']['metabox_enabled'] = new Element_Checkbox(
				'<b>' . __( 'Add as admin post meta box to the edit screen', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][metabox_enabled]',
				array( 'metabox_enabled' => '<b>' . __( 'Add this field to the MetaBox', 'buddyforms' ) . '</b>' ),
				array(
	...
	$form_fields['advanced']['slug'] = new Element_Textbox(
		'<b>' . __( 'Slug', 'buddyforms' ) . '</b>' . sprintf( '<small>(%s)</small>', __( 'optional', 'buddyforms' ) ),
	...
	switch ( sanitize_title( $field_type ) ) {
		...
		case 'text':
			...
			$form_fields['validation']['validation_minlength'] = new Element_Number(
				'<b>' . __( 'Validation Min Length', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][validation_minlength]',
				array(
			...
			$form_fields['validation']['validation_maxlength'] = new Element_Number(
				'<b>' . __( 'Validation Max Length', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][validation_maxlength]',
				array(
		...
		case 'country':
			...
			$form_fields['general']['country_list']    = new Element_Textarea(
				'<b>' . __( 'Country JSON:', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][country_list]',
				array(
			...
			$form_fields['general']['link_with_state'] = new Element_Checkbox(
				...
				array( 'link' => '<b>' . __( 'Show the State from the selected Country', 'buddyforms' ) . '</b>' ),
				array(
		...
		case 'state':
			...
			$form_fields['general']['state_list'] = new Element_Textarea(
				'<b>' . __( 'State JSON:', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][state_list]',
				array(
		...
		case 'user_login':
			...
			$form_fields['general']['hide_if_logged_in'] = new Element_Checkbox(
				...
				array( 'hide' => '<b>' . __( 'Hide for logged in user', 'buddyforms' ) . '</b>' ),
				array(
		...
		case 'user_email':
			...
			$form_fields['validation']['validation_email_msj'] = new Element_Textbox(
				'<b>' . __( 'Invalid Email Message', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][validation_email_msj]',
				array(
		...
		case 'user_pass':
			...
			$form_fields['general']['new_password_placeholder']     = new Element_Textbox(
				'<b>' . __( 'New Password Placeholder', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][new_password_placeholder]',
				array(
					'data'     => $field_id,
			...
			$form_fields['general']['confirm_password_placeholder'] = new Element_Textbox(
				'<b>' . __( 'Confirm Password Placeholder', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][confirm_password_placeholder]',
				array(
					'data'     => $field_id,
		...
		case 'captcha':
			...
			$form_fields['general']['captcha_site_key']    = new Element_Textbox(
				'<b>' . __( 'Site Key.', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][captcha_site_key]',
				array(
					'data'      => $field_id,
			...
			$form_fields['general']['captcha_private_key'] = new Element_Textbox(
				'<b>' . __( 'Private Key', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][captcha_private_key]',
				array(
					'data'     => $field_id,
			...
			$form_fields['general']['captcha_language'] = new Element_Textbox(
				'<b>' . __( 'Language', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][captcha_language]',
				array(
					'data'      => $field_id,
			...
			$form_fields['general']['captcha_v3_action'] = new Element_Textbox(
				'<b>' . __( 'Action', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][captcha_v3_action]',
				array(
					'data'      => $field_id,
			...
			$form_fields['general']['captcha_v3_score'] = new Element_Number(
				'<b>' . __( 'Score', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][captcha_v3_score]',
				array(
					'data'      => $field_id,
					'step'      => '0.1',
					'max'       => '1.0',
			...
			$form_fields['general']['html'] = new Element_HTML( sprintf( '<p><b>%s<b><p>', __( 'reCaptcha is only visible to logged-out users. Logged-in users do not need to be checked.', 'buddyforms' ) ) );
		...
		case 'textarea':
			...
			$form_fields['advanced']['generate_textarea'] = new Element_Textbox(
				'<b>' . __( 'Generate textarea', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][generate_textarea]',
				array(
					'data'      => $field_id,
		...
		case 'post_excerpt':
			...
			$form_fields['advanced']['generate_post_excerpt'] = new Element_Textarea(
				'<b>' . __( 'Generate Post Excerpt', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][generate_post_excerpt]',
				array(
		...
		case 'number':
			...
			$form_fields['validation']['validation_min'] = new Element_Number(
				'<b>' . __( 'Validation Min Value', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][validation_min]',
				array(
			...
			$form_fields['validation']['validation_max'] = new Element_Number(
				'<b>' . __( 'Validation Max Value', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][validation_max]',
				array(
		...
		case 'dropdown':
			...
			$form_fields['general']['frontend_reset'] = new Element_Checkbox(
				...
				array( 'frontend_reset' => __( 'Enable Frontend Reset Option', 'buddyforms' ) ),
				array(
		...
		case 'upload':
			...
			$form_fields['general']['upload_file_limts'] = new Element_Number(
				'<b>' . __( 'Max File Size in MB', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][file_limit]',
				array(
			...
			$form_fields['general']['upload_accepted_files_label'] = new Element_Textarea(
				'<b>' . __( 'Allowed File Types Resume', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][html]',
				array(
					'value'    => $html,
					'readonly' => 'readonly',
				)
			...
			$form_fields['validation']['ensure_amount']         = new Element_Checkbox(
				...
				array( 'ensure_amount' => '<b>' . __( 'Files in the field should be equals to Max Number of Files. ', 'buddyforms' ) . '</b>' ),
				array(
					'id'    => 'ensure_amount' . $field_id,
					'value' => $ensure_amount,
				)
			);
			$form_fields['validation']['upload_multiple_files'] = new Element_Number(
				'<b>' . __( 'Max number of files that will be handled', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][multiple_files]',
				array(
			...
			$element_delete = new Element_Checkbox(
				...
				array( 'delete' => __( 'Remove Files when Entry is deleted. ', 'buddyforms' ) ),
				array(
			...
			$element_delete = new Element_Checkbox(
				...
				array( 'upload_from_url' => __( 'Upload Images From Url. ', 'buddyforms' ) ),
				array(
		...
		case 'date':
			...
			$form_fields['general']['element_date_format'] = new Element_Textbox(
				'<b>' . __( 'Date Format', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][element_date_format]',
				array(
			...
			$form_fields['general']['enable_time']         = new Element_Checkbox(
				...
				array( 'enable_time' => '<b>' . __( 'Include the Time Picker in the element', 'buddyforms' ) . '</b>' ),
				array(
					'value' => $enable_time,
				)
			);
			$element_time_format                           = isset( $customfield['element_time_format'] ) ? stripcslashes( $customfield['element_time_format'] ) : 'hh:mm tt';
			$form_fields['general']['element_time_format'] = new Element_Textbox(
				'<b>' . __( 'Time Format', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][element_time_format]',
				array(
			...
			$form_fields['general']['element_time_step'] = new Element_Number(
				'<b>' . __( 'Time Step', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][element_time_step]',
				array(
			...
			$form_fields['validation']['element_date_invalid_format'] = new Element_Textbox(
				'<b>' . __( 'Invalid Format Message', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][element_date_invalid_format]',
				array(
		...
		case 'time':
			...
			$form_fields['general']['element_time_hour_step'] = new Element_Number(
				'<b>' . __( 'Hour Step', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][element_time_hour_step]',
				array(
			...
			$form_fields['general']['element_time_minute_step'] = new Element_Number(
				'<b>' . __( 'Minute Step', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][element_time_minute_step]',
				array(
		...
		case 'post_formats':
			...
			$form_fields['advanced']['hidden_field'] = new Element_Checkbox(
				...
				array( 'hidden_field' => '<b>' . __( 'Make this field Hidden', 'buddyforms' ) . '</b>' ),
				array(
					'value' => $hidden,
					'class' => 'bf_hide_if_post_type_none',
				)
		...
		case 'tags':
			...
			$form_fields['general']['taxonomy_placeholder'] = new Element_Textbox(
				'<b>' . __( 'Taxonomy Placeholder', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][taxonomy_placeholder]',
				array(
					'data'      => $field_id,
			...
			$form_fields['general']['multiple'] = new Element_Checkbox(
				...
				array( 'multiple' => '<b>' . __( 'Multiple', 'buddyforms' ) . '</b>' ),
				array(
					'value' => $multiple,
					'class' => 'bf_hide_if_post_type_none',
				)
			...
			$form_fields['validation']['maximumSelectionLength'] = new Element_Number(
				'<b>' . __( 'Limit Selections', 'buddyforms' ) . '</b>',
			...
			$form_fields['general']['create_new_tax'] = new Element_Checkbox(
				...
				array( 'user_can_create_new' => '<b>' . __( 'User can create new', 'buddyforms' ) . '</b>' ),
				array(
			...
			$form_fields['general']['ajax'] = new Element_Checkbox(
				...
				array( 'is_ajax' => '<b>' . __( 'Enabled Ajax', 'buddyforms' ) . '</b>' ),
				array(
					'value' => $is_ajax,
					'data'  => $field_id,
			...
			$form_fields['general']['minimumInputLength'] = new Element_Number(
				'<b>' . __( 'Minimum characters ', 'buddyforms' ) . '</b>',
...
JS;
		...
		case 'title':
			...
			$form_fields['advanced']['generate_title'] = new Element_Textbox(
				'<b>' . __( 'Generate Title', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][generate_title]',
				array(
		...
		case 'content':
			...
			$form_fields['advanced']['generate_content'] = new Element_Textarea(
				'<b>' . __( 'Generate Content', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][generate_content]',
				array(
		...
		case 'status':
			...
			$status_date_format_element = new Element_Textbox(
				'<b>' . __( 'Date Format', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][status_date_format]',
				array(
			...
			$status_time_format_element = new Element_Textbox(
				'<b>' . __( 'Time Format', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][status_time_format]',
				array(
		...
		case 'featured_image':
			...
			$form_fields['general']['max_file_size'] = new Element_Number(
				'<b>' . __( 'Max File Size in MB', 'buddyforms' ) . '</b>',
				'buddyforms_options[form_fields][' . $field_id . '][max_file_size]',
				array(
		...
		case 'form_actions':
			...
			$form_fields['general']['disabled_publish'] = new Element_Checkbox(
				...
				array( 'disabled_publish' => '<b>' . __( 'Check to disabled', 'buddyforms' ) . '</b>' ),
				array(
	...
	if ( is_array( $args ) ) {
		...
		echo wp_kses( $field_html, buddyforms_wp_kses_allowed_atts() );
		die();
	}
	...
	<table class="wp-list-table widefat posts fixed">
		...
		if ( isset( $form_fields ) ) {
			foreach ( $form_fields as $key => $field ) {
				...
				switch ( $type ) {
					case 'html':
						echo '<tr id="table_row_' . esc_attr( $field_id ) . '_' . esc_attr( $key ) . '" class="' . esc_attr( $classes ) . '"><td colspan="2">';
						$field->render();
						echo '</td></tr>';
						break;
					case 'hidden':
					...
					default:
						?>
						<tr id="table_row_<?php echo esc_attr( $field_id ); ?>_<?php echo esc_attr( $key ); ?>" class="<?php echo esc_attr( $classes ); ?>">
							<th scope="row">
								<label for="form_title"><?php echo wp_kses_post( $field->getLabel() ); ?></label>
							</th>
							<td>
								<?php echo wp_kses( $field->render(), buddyforms_wp_kses_allowed_atts() ); ?>
								<p class="description"><?php echo wp_kses_post( $field->getShortDesc() ); ?></p>
							</td>
						</tr>
						<?php
						break;
