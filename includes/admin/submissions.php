...
class BuddyFormsSubmissionPage {
	...
	public function buddyforms_submissions_screen() {
		...
		$selected_form   = '';
		$selected_author = isset( $_GET['submission_author'] ) ? sanitize_text_field( wp_unslash( $_GET['submission_author'] ) ) : 'all';
		if ( isset( $_GET['form_slug'] ) ) {
			$current_screen->set_parentage( $parent_file );
			$current_screen->render_screen_meta();
			$selected_form = buddyforms_sanitize_slug( wp_unslash( $_GET['form_slug'] ) );
		}
		?>
		...
		<?php if ( isset( $_GET['form_slug'] ) ) : ?>
			<?php if ( $this->has_the_capability( buddyforms_sanitize_slug( wp_unslash( $_GET['form_slug'] ) ) ) ) : ?>
				<?php if ( ! isset( $_GET['entry'] ) && isset( $_REQUEST['page'] ) ) { ?>
				...
				if ( isset( $_GET['action'] ) && isset( $_GET['entry'] ) ) {
					$form_slug = get_post_meta( sanitize_key( $_GET['entry'] ), '_bf_form_slug', true );
					$post_id   = sanitize_key( $_GET['entry'] );
	...
	function redirect_after_delete() {
		...
		$form_slug = isset( $_GET['form_slug'] ) ? buddyforms_sanitize_slug( wp_unslash( $_GET['form_slug'] ) ) : '';
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'buddyforms_submissions' && isset( $_GET['entry'] ) ) {
			if ( ! get_post( sanitize_key( $_GET['entry'] ) ) ) {
				wp_redirect( '?post_type=buddyforms&page=buddyforms_submissions&form_slug=' . buddyforms_sanitize_slug( wp_unslash( $_GET['form_slug'] ) ) );
			}
		}
	}
...
class BuddyForms_Submissions_List_Table extends WP_List_Table {
	...
	function column_ID( $item ) {
		...
		if ( isset( $buddyforms[ $_GET['form_slug'] ]['post_type'] ) && isset( $_REQUEST['page'] ) && sanitize_text_field( wp_unslash( $buddyforms[ $_GET['form_slug'] ]['post_type'] ) ) == 'bf_submissions' ) {
			$actions['edit'] = sprintf( '<a href="?post_type=buddyforms&page=%s&action=%s&entry=%s&form_slug=%s">%s</a>', buddyforms_sanitize_mixed( wp_unslash( $_REQUEST['page'] ) ), 'edit', $item->ID, buddyforms_sanitize_mixed( wp_unslash( $_GET['form_slug'] ) ), __( 'View Submission', 'buddyforms' ) );
		}
	...
	function column_default( $item, $column_name ) {
		...
		$bf_field = buddyforms_get_form_field_by_slug( buddyforms_sanitize_slug( wp_unslash( $_GET['form_slug'] ) ), $column_name );
		if ( $bf_field !== false ) {
	...
	function get_columns() {
		...
		if ( isset( $_GET['form_slug'] ) && isset( $buddyforms[ $_GET['form_slug'] ]['form_fields'] ) ) {
			foreach ( $buddyforms[ buddyforms_sanitize_slug( wp_unslash( $_GET['form_slug'] ) ) ]['form_fields'] as $key => $field ) {
	...
	function query( $author_to_filter = array() ) {
		...
		if ( isset( $_GET['form_slug'] ) ) {
			...
			$customvalue = ! empty( $_GET['form_slug'] ) ? buddyforms_sanitize_slug( wp_unslash( $_GET['form_slug'] ) ) : '';
