	...
	if ( isset( $form['form_fields'] ) ) {
		...
		foreach ( $form['form_fields'] as $key => $form_field ) {
			...
			if ( isset( $form_field['validation_minlength'] ) && $form_field['validation_minlength'] > 0 ) {
				if ( strlen( trim( sanitize_text_field( wp_unslash( $_POST[ $form_field['slug'] ] ) ) ) ) < $form_field['validation_minlength'] ) {
					$valid                    = false;
			...
			if ( isset( $form_field['validation_maxlength'] ) && $form_field['validation_maxlength'] > 0 ) {
				if ( strlen( trim( sanitize_text_field( wp_unslash( $_POST[ $form_field['slug'] ] ) ) ) ) > $form_field['validation_maxlength'] ) {
					$valid                    = false;
