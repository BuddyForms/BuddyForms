									...
									<li>
										<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>"
											...
											?>'))  this.parentNode.submit(); return false;"><i
													class="dashicons dashicons-no"></i> <?php _efs( 'delete-account', $slug ) ?>
											</a>
										</form>
									</li>
									<?php if ( $is_paying ) : ?>
													...
													<a href="#"
													   onclick="if (confirm('<?php printf( __fs( 'downgrade-x-confirm', $slug ), $plan->title, human_time_diff( time(), strtotime( $license->expiration ) ) ) ?> <?php if ( ! $license->is_block_features ) {
														   ...
														   printf( __fs( 'after-downgrade-blocking', $slug ), $plan->title );
															...
															class="dashicons dashicons-download"></i> <?php _efs( 'downgrade', $slug ) ?>
													</a>
												</form>
											</li>
										<?php endif ?>
										<li>
											...
											<a href="<?php echo $fs->get_upgrade_url() ?>"><i
													class="dashicons dashicons-grid-view"></i> <?php _efs( 'change-plan', $slug ) ?>
											</a>
										</li>
											...
											<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>"
												...
												<a href="#"
												   onclick="if (confirm('<?php _efs( 'cancel-trial-confirm' ) ?>')) this.parentNode.submit(); return false;"><i
														class="dashicons dashicons-download"></i> <?php _efs( 'cancel-trial', $slug ) ?>
												</a>
											</form>
										</li>
									<?php endif ?>
									<li>
										...
										<form action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>"
											...
											<a href="#" onclick="this.parentNode.submit(); return false;"><i
													class="dashicons dashicons-image-rotate"></i> <?php _efs( 'sync', $slug ) ?>
											</a>
										</form>
									</li>
										...
										<tr class="fs-field-<?php echo $p['id'] ?><?php if ( $odd ) : ?> alternate<?php endif ?>">
											...
											<td<?php if ( 'plan' === $p['id'] ) {
												...
												<?php if ( 'email' === $p['id'] && ! $user->is_verified() ) : ?>
													<label
														class="fs-tag fs-warn"><?php _efs( 'not-verified', $slug ) ?></label>
												<?php endif ?>
												<?php if ( 'plan' === $p['id'] ) : ?>
													<?php if ( $fs->is_trial() ) : ?>
														<label
															class="fs-tag fs-success"><?php _efs( 'trial', $slug ) ?></label>
													<?php endif ?>
															...
															<form
																...
																<?php if ( $show_upgrade ) : ?>
																	<a class="button activate-license-trigger <?php echo $slug ?>"
																	   href="#"><?php _efs( 'activate-license', $slug ) ?></a>
																<?php endif ?>
												...
												<?php elseif ( 'version' === $p['id'] && $fs->has_paid_plan() ) : ?>
													...
													<?php elseif ( $fs->can_use_premium_code() ) : ?>
														<label
															class="fs-tag fs-warn"><?php _efs( 'free-version' ) ?></label>
													<?php endif ?>
												<?php endif ?>
											</td>
											<?php if ( 'plan' !== $p['id'] ) : ?>
												<td class="fs-right">
													...
													elseif ( in_array( $p['id'], array(
														...
														<button
															class="button button-small"><?php _efs( 'show', $slug ) ?></button>
														<?php
