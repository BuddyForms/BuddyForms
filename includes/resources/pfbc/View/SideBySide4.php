...
class View_SideBySide4 extends FormView {
	...
	public function renderElement( $element ) {
		...
		if ( $this->sharedCount == 0 ) {
			echo '<div class="row form-group elem-' . esc_attr( $element->getAttribute( 'id' ) ) . '"> ', wp_kses( $this->renderLabel( $element ), buddyforms_form_allowed_tags() );
		}
	...
	protected function renderLabel( Element $element ) {
		...
		echo sprintf( '<label class="text-right-lg col-xs-12 col-md-4 form-control-label" for="%s">%s</label>', esc_attr( $element->getAttribute( 'id' ) ), esc_html( $label ) );
	}
