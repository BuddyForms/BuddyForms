...
class Element_Upload extends Element_Textbox {
	...
	public static function upload_process_field_submission( $field_slug, $field_type, $field, $post_id, $form_slug, $args, $action ) {
		...
		if ( $field_type === 'upload' || $field_type === 'featured_image' ) {
			$key_value = isset( $_POST[ $field_slug ] ) ? buddyforms_sanitize( '', wp_unslash( $_POST[ $field_slug ] ) ) : '';
	...
	public function render() {
		...
		$action = isset( $_GET['action'] ) ? buddyforms_sanitize( '', wp_unslash( $_GET['action'] ) ) : '';
		$entry  = isset( $_GET['entry'] ) ? buddyforms_sanitize( '', wp_unslash( $_GET['entry'] ) ) : '';
		$page   = isset( $_GET['page'] ) ? buddyforms_sanitize( '', wp_unslash( $_GET['page'] ) ) : '';

		// If Entry is empty we check if we are in a edit entry page
		if ( empty( $entry ) ) {
			$entry = isset( $_GET['post'] ) ? buddyforms_sanitize( '', wp_unslash( $_GET['post'] ) ) : '';
		}
