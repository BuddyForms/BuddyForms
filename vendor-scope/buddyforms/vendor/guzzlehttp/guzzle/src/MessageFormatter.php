 ...
 * @final
 */
class MessageFormatter implements \tk\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\tk\Psr\Http\Message\RequestInterface $request, ?\tk\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \tk\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \tk\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
                ...
                case 'req_body':
                    ...
                    break;
                case 'res_body':
                    if (!$response instanceof \tk\Psr\Http\Message\ResponseInterface) {
                        $result = 'NULL';
                        break;
                    }
     ...
     */
    private function headers(\tk\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
