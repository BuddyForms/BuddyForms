    ...
    public function __invoke(\tk\Psr\Http\Message\RequestInterface $request, array $options) : \tk\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if (\is_string($uri) && ($type = \tk\GuzzleHttp\Psr7\MimeType::fromFilename($uri))) {
        ...
        return $fn(\tk\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify), $options);
    }
    /**
     * Add expect header
     */
    private function addExpectHeader(\tk\Psr\Http\Message\RequestInterface $request, array $options, array &$modify) : void
    {
