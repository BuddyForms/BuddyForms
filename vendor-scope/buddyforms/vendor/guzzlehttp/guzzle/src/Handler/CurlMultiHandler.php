...
use tk\GuzzleHttp\Promise\Promise;
...
class CurlMultiHandler
{
    ...
    {
        $this->factory = $options['handle_factory'] ?? new \tk\GuzzleHttp\Handler\CurlFactory(50);
        if (isset($options['select_timeout'])) {
            $this->selectTimeout = $options['select_timeout'];
        } elseif ($selectTimeout = \tk\GuzzleHttp\Utils::getenv('GUZZLE_CURL_SELECT_TIMEOUT')) {
    ...
    public function __invoke(\tk\Psr\Http\Message\RequestInterface $request, array $options) : \tk\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $id = (int) $easy->handle;
        $promise = new \tk\GuzzleHttp\Promise\Promise([$this, 'execute'], function () use($id) {
    ...
    public function tick() : void
    {
        ...
        if ($this->delays) {
            $currentTime = \tk\GuzzleHttp\Utils::currentTime();
    ...
    {
        $queue = \tk\GuzzleHttp\Promise\Utils::queue();
    ...
    private function addRequest(array $entry) : void
    {
        ...
        if (empty($easy->options['delay'])) {
            ...
            $this->delays[$id] = \tk\GuzzleHttp\Utils::currentTime() + $easy->options['delay'] / 1000;
        }
    }
    /**
    ...
    private function processMessages() : void
    {
        while ($done = \curl_multi_info_read($this->_mh)) {
            ...
            $entry['deferred']->resolve(\tk\GuzzleHttp\Handler\CurlFactory::finish($this, $entry['easy'], $this->factory));
        }
    }
